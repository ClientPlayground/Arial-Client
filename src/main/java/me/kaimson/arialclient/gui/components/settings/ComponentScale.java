...
public class ComponentScale extends ComponentSetting {
    ...
    public ComponentScale(int x, int y, int width, int height, float min, float max, float step, float current, Feature setting, Feature s) {
        ...
        this.display = "";
        this.displayString = String.valueOf(setting.getScale());
        this.sliderValue = MathUtil.normalizeValue(setting.getScale(), min, max, step);
        this.s = s;
    }
    ...
    public void render(int xOffset, int yOffset, int mouseX, int mouseY, float partialTicks) {
        ...
        GuiUtils.drawString(s.getTranslation(), x + xOffset - mc.fontRendererObj.getStringWidth(s.getTranslation()) - 3, y + yOffset - 3);
    ...
    public void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        this.dragging = false;
        Config.INSTANCE.setScale(setting, MathUtil.denormalizeValue(sliderValue, min, max, step));
