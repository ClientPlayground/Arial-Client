...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void loadShader(ResourceLocation resourceLocationIn)
    {
        if (OpenGlHelper.isFramebufferEnabled())
        {
            try
            {
                this.theShaderGroup = new ShaderGroup(this.mc.getTextureManager(), resourceManager, this.mc.getFramebuffer(), resourceLocationIn);
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping())
        {
            ...
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                if (Perspective.isEnabled()) {
                    GlStateManager.rotate(Perspective.cameraYaw + (Perspective.cameraYaw - Perspective.cameraYaw) * partialTicks + 180.0F, 0.0F, -1.0F, 0.0F);
                    GlStateManager.rotate(Perspective.cameraPitch + (Perspective.cameraPitch - Perspective.cameraPitch) * partialTicks, -1.0F, 0.0F, 0.0F);
                } else {
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                if (Perspective.isEnabled())
                {
                    f1 = Perspective.cameraYaw;
                    f2 = Perspective.cameraPitch;
                }
                ...
                if (Perspective.isEnabled())
                {
                    GlStateManager.rotate(Perspective.cameraPitch - f2, 1.0F, 0.0F, 0.0F);
                    GlStateManager.rotate(Perspective.cameraYaw - f1, 0.0F, 1.0F, 0.0F);
                    GlStateManager.translate(0.0F, 0.0F, (float)(-d3));
                    GlStateManager.rotate(f1 - Perspective.cameraYaw, 0.0F, 1.0F, 0.0F);
                    GlStateManager.rotate(f2 - Perspective.cameraPitch, 1.0F, 0.0F, 0.0F);
                } else {
...
/*        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists())
        {*/
            if (!this.mc.gameSettings.debugCamEnable)
            {
                ...
                if (Perspective.isEnabled())
                {
                    f6 = Perspective.cameraYaw + (Perspective.cameraYaw - Perspective.cameraYaw) * partialTicks + 180.0F;
                    f7 = Perspective.cameraPitch + (Perspective.cameraPitch - Perspective.cameraPitch) * partialTicks;
                }
...
/*        }
    ...
    public void renderHand(float p_renderHand_1_, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_)
    {
        ...
        if (Feature.ANIMATIONS.isEnabled())
            if (Feature.ANIMATIONS_BUILD.isEnabled()) {
                ...
                {
                    PlayerControllerMP.isHittingBlock = false; //Applies block build
                }
            }
    }

    public void swingItem(EntityLivingBase e)
    {
        ItemStack stack = e.getHeldItem();

        if (stack != null && stack.getItem() != null)
        {
//          if (stack.getItem().onEntitySwing((Minecraft.getMinecraft()).thePlayer, stack))
//            return;  }
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        if (this.mc.inGameHasFocus && flag)
        {
            ...
            if (this.mc.gameSettings.smoothCamera)
            {
                ...
                if (Perspective.isEnabled())
                {
                    Perspective.cameraYaw += f2 / 8.0F;
                    Perspective.cameraPitch += f3 / 8.0F;

                    if (Math.abs(Perspective.cameraPitch) > 90.0F)
                    {
                        Perspective.cameraPitch = (Perspective.cameraPitch > 0.0F ? 90.0F : -90.0F);
                    }
                } else {
